    ...
    return (
        <PanelSeparator
            ...
            className={cn(
                ...
                // The vertical-group overrides target the IMMEDIATE
                // parent only. With the descendant combinator (`_`)
                // they used to match any ancestor vertical group too,
                // so a horizontal Group nested inside a vertical Group
                // (e.g. FileBrowser's preview split inside ProjectShell's
                // vertical drawer split) had its handle inherit
                // `w-full` and devour the inner row's width — both
                // panels collapsed to 0. Use the child combinator
                // (`>`) so the override only fires when the handle
                // sits directly under a vertical Group.
                "[[data-panel-group-direction=vertical]>&]:h-px [[data-panel-group-direction=vertical]>&]:w-full",
                "[[data-panel-group-direction=vertical]>&]:after:left-0 [[data-panel-group-direction=vertical]>&]:after:h-1.5 [[data-panel-group-direction=vertical]>&]:after:w-full [[data-panel-group-direction=vertical]>&]:after:-translate-y-1/2 [[data-panel-group-direction=vertical]>&]:after:translate-x-0",
                className,
            )}
            {...props}
        >
