...
// the row metadata flows through here so the RecordingsPage can list,
...
// fetchRecordingCastBlob pulls the raw .cast file via authFetch and
// returns it as a Blob. asciinema-player's fetch loader can't carry a
// Bearer header, so we hand it a blob URL that points at the bytes
// we already pulled.
