...
// HostsTableSkeleton matches the populated table's column widths so
// the swap from "loading" to "loaded" is a content swap with no
// layout shift. Renders 6 placeholder rows — visually enough to read
// as "things are coming" without taking the whole viewport. Each row
// is tagged data-testid="hosts-row-skeleton" so the e2e spec can
// detect the loading state without depending on Tailwind class names.
function HostsTableSkeleton() {
    ...
    return (
        <Card padding={0}>
            <Table>
                ...
                <TableBody>
                    {rows.map((_, i) => (
                        <TableRow key={i} data-testid="hosts-row-skeleton">
                            <TableCell>
