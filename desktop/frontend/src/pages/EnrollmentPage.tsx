...
function IssuedInstallDialog({
    ...
    // Two parallel bootstrap surfaces:
    //
    ...
    // picks one or the other but never both.
            ...
            <DialogContent className="sm:max-w-[640px]">
                ...
                <Tabs value={tab} onValueChange={(v) => setTab(v as "script" | "bundle")}>
                    ...
                    <TabsContent value="script" className="mt-3 space-y-2">
                        <div className="text-xs text-text-muted">
                            Run on the target machine. Downloads the agent binary, then enrols
                            using the freshly-minted PAT.
                        </div>
                    ...
                    <TabsContent value="bundle" className="mt-3 space-y-2">
                        <div className="text-xs text-text-muted">
                            ...
                            <Mono>| sh</Mono> isn't appropriate. <Mono>curl</Mono> returns a
                            self-contained <Mono>pinst_</Mono> token; pipe it straight to{" "}
                            <Mono>platypus-agent</Mono> (binary must already be on the host).
                        </div>
                        <pre className="rounded border border-border bg-surface p-3 font-mono text-xs break-all whitespace-pre-wrap">
                            {bundleOneLiner}
                        </pre>
                ...
                <DialogFooter>
                    <Button
                        variant="outline"
                        onClick={() =>
                            copy(tab === "script" ? result?.install_command ?? "" : bundleOneLiner)
                        }
                    >
