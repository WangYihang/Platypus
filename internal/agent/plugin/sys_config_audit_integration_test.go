...
func installSysConfigAudit(t *testing.T) *plugin.Registry {
	t.Helper()
	wasm, err := os.ReadFile(sysConfigAuditWasmPath())
	if err != nil {
		t.Skipf("sys_config_audit.wasm not built (%v) — run `cargo build --release --target wasm32-unknown-unknown` in example/plugins/system/sys-config-audit/", err)
	}
	manifestBytes, err := os.ReadFile(filepath.Join("..", "..", "..",
		"example", "plugins", "system", "sys-config-audit", "plugin.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func sysConfigAuditWasmPath() string {
	return filepath.Join("..", "..", "..", "example", "plugins", "system", "sys-config-audit",
		"target", "wasm32-unknown-unknown", "release", "sys_config_audit.wasm")
}
