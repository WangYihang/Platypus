...
func installSysNetWindows(t *testing.T) *plugin.Registry {
	t.Helper()
	const id = "com.platypus.sys-net-windows"
	const ver = "1.0.0"
	wasm := stagedWasmBytes(t, id, ver, "sys_net_windows.wasm")
	...
	sig, err := plugin.Sign(sk, wasm, plugin.DefaultTrustedComment("sys_net_windows.wasm"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSysNetWindows_Manifest(t *testing.T) {
	manifestBytes := stagedManifestBytes(t, "com.platypus.sys-net-windows", "1.0.0")
...
// Placeholder for the windows round-trip test. Real assertions
// require a windows runner; on non-windows the install path is
// already covered by TestSysNetWindows_Manifest.
func TestSysNetWindows_RoundTrip(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("sys-net-windows shells to powershell.exe; runs on windows only")
	}
	_ = installSysNetWindows(t)
	// On a real windows runner we'd invoke list_listeners and
	// assert at least one port is bound. Left as a placeholder.
}
