...
func installSysPkgWindows(t *testing.T) *plugin.Registry {
	t.Helper()
	const id = "com.platypus.sys-pkg-windows"
	const ver = "1.0.0"
	wasm := stagedWasmBytes(t, id, ver, "sys_pkg_windows.wasm")
	...
	sig, err := plugin.Sign(sk, wasm, plugin.DefaultTrustedComment("sys_pkg_windows.wasm"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSysPkgWindows_Manifest(t *testing.T) {
	manifestBytes := stagedManifestBytes(t, "com.platypus.sys-pkg-windows", "1.0.0")
...
func TestSysPkgWindows_InstallSucceeds(t *testing.T) {
	_ = installSysPkgWindows(t)
}
