...
type Identity struct {
	...
	// CertPEM is the project-CA-signed leaf cert the agent received
	// at enrollment, when the Identity was loaded from cert material
	// via LoadIdentityFromCert. Empty for identities generated by
	// NewIdentity / LoadOrCreateIdentity — in that case the NodeID
	// is the self-certifying DeriveNodeID(PublicKey) hash. When
	// CertPEM is set, NodeID comes from the cert's
	// "platypus://agent/<id>" SAN instead, and the cert bytes are
	// published via gossip so peers can verify membership against
	// the project CA.
	CertPEM []byte
}
...
// LoadIdentityFromCert constructs an Identity from the PEM bytes of
// an agent cert + its PKCS#8 private key, as persisted by
// internal/agent.SaveIdentity. The NodeID is extracted from the
// cert's "platypus://agent/<id>" URI SAN (not DeriveNodeID), so
// mesh membership chains back to whoever issued the cert rather
// than being self-certifying.
//
// The function validates that:
//   - the cert parses as an Ed25519 leaf with the expected SAN
//   - the PKCS#8 key parses as an Ed25519 private key
//   - the private key's derived public key matches the cert's —
//     belt-and-braces against accidentally mixing a cert with the
//     wrong key on disk
//
