...
func TestLSDBIngestRejectsBadSignature(t *testing.T) {
	...
	db := newLSDB()
	if _, err := db.Ingest(lsa, testCAPool); err == nil {
...
func TestLSDBIngestIgnoresOlderSeq(t *testing.T) {
	alice := mustIdentity(t)
	db := newLSDB()
	if _, err := db.Ingest(buildSignedLSA(t, alice, 5, "p1"), testCAPool); err != nil {
		t.Fatalf("ingest 5: %v", err)
	}
	changed, err := db.Ingest(buildSignedLSA(t, alice, 3, "p1"), testCAPool)
	if err != nil {
...
func TestComputeRoutesLinearChain(t *testing.T) {
	...
	if _, err := lsdb.Ingest(buildSignedLSA(t, b, 1, a.NodeID, c.NodeID), testCAPool); err != nil {
		t.Fatalf("ingest B: %v", err)
	}
	// C's LSA says C is connected to B.
	if _, err := lsdb.Ingest(buildSignedLSA(t, c, 1, b.NodeID), testCAPool); err != nil {
